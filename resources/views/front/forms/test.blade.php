        ...
        <div class="container">
            ...
            <div class="row">
                ...
                <div class="col-lg-12" data-aos="fade-up" data-aos-delay="300">
                    ...
                    <form action="{{ route('front.forms.submit', ['category' => $category->slug]) }}" method="POST">
                        @csrf
                        ...
                        <div class="row">
                            <div class="col-lg-6">
                                ...
                                <div class="form-group">
                                    <label for="test_category_id">Jenis Test</label>
                                    @forelse($testCategories as $testCat)
                                        <div class="form-check">
                                            <input class="form-check-input" type="radio" name="test_category_id" id="testType{{ $testCat->id }}" value="{{ $testCat->id }}" required>
                                            <label class="form-check-label" for="testType{{ $testCat->id }}">{{ $testCat->name }}</label>
                                        </div>
                                    @empty
                                        <p class="text-danger">Tidak ada kategori tes tersedia</p>
                                    @endforelse
                                </div>
                            ...
                            <div class="col-lg-6">
                                <!-- Pilih Jadwal -->
                                <div class="form-group">
                                    <label for="schedule_id">Pilih Jadwal Test</label>
                                    <select class="form-control" name="schedule_id" required>
                                        <option value="">Pilih Jadwal...</option>
                                        @foreach($schedules as $schedule)
                                            <option value="{{ $schedule->id }}">{{ $schedule->formatted_date }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                ...
                                <!-- Kepentingan Tes -->
                                <div class="form-group">
                                    <label for="purpose">Kepentingan Tes</label>
                                    <input type="text" class="form-control" name="purpose" placeholder="Masukkan kepentingan tes" required>
                                </div>
                            </div>
                        </div>
